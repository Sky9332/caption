...
router.post(
  "/",
  async (req: Request<{}, {}, TranscodeRequest>, res: Response) => {
    try {
      const { inputUrl, outputFormat, captions, captionSettings, audioUrl } =
        req.body;
      ...
      if (audioUrl) {
        command = command
          .input(audioUrl)
          .audioCodec("aac")
          .outputOptions(["-map 0:v", "-map 1:a"]); // Use video from first input, audio from second input
      }
      ...
      if (captions && captionSettings) {
        ...
        const alignment =
          captionSettings.position === "top"
            ? 6
            : captionSettings.position === "middle"
              ? 10
              : 2;
      ...
      command
        ...
        .on("error", (err) => {
          ...
          res
            .status(500)
            .json({ error: "Transcoding failed", details: err.message });
          // Clean up on error
